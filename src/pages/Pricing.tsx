  ...
  return (
    <section className="py-16">
      <div className="container">
        ...
        <p className="mx-auto mt-3 max-w-md text-center text-muted-foreground text-sm mb-10">
        ...
        <div className="mb-10 flex items-center justify-between max-w-md mx-auto px-4">
          {[
            ...
            { label: "Fill Details", active: false, done: true },
            { label: "Select Plan", active: true, done: false },
            { label: "Go Live", active: false, done: false },
          ].map((step, idx, arr) => (
            <div key={step.label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-semibold ${step.active
              ...
              {idx < arr.length - 1 && (
                <div
                  className={`h-[2px] flex-1 mx-2 -mt-4 ${step.done ? "bg-primary" : "bg-muted"
                    }`}
                />
              )}
            </div>
        ...
        <div className="mt-14 grid gap-8 md:grid-cols-3">
          {plans.map((plan) => {
            const waText = encodeURIComponent(
              `Hi! I'm interested in the ${plan.name} plan (${plan.price}${plan.period}). Please help me get started with Apna Website.`
            );
            const waUrl = `https://wa.me/919892986314?text=${waText}`;

            return (
              <div
                key={plan.name}
                className={`relative flex flex-col rounded-2xl border p-8 transition-all hover:-translate-y-1 ${plan.highlighted
      ...
      <section className="py-20 bg-gradient-to-b from-transparent to-primary/5">
        <div className="container">
          ...
          <p className="mx-auto mt-3 max-w-xl text-center text-muted-foreground mb-14">
            Create and preview your website for free. Only pay when you're ready to go live.
          </p>
          <HowWeWork />
        </div>
      </section>
